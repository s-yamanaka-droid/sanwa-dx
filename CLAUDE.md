...
ページリロードをまたいで保持される設定・データ一覧：

| キー | 保存内容 | 使用コンポーネント |
|---|---|---|
| `sanwa_alertEmail` | 在庫不足アラートの送信先メールアドレス | `Inventory.jsx` |
| `sanwa_colFormat` | Excel出力の列設定（ON/OFF・ヘッダー名） | `Inventory.jsx` |
| `sanwa_orderHistory` | 発注履歴（全件。配列をJSON文字列で保存） | `Inventory.jsx` |
| `sanwa_geminiKey` | Gemini API キー | `PdfReader.jsx` |
...
- [ ] 複数案件の管理（プロジェクト一覧画面・切替機能）
- [ ] Supabase 連携（発注履歴・設定をクラウド保存し複数端末対応）
- [ ] 見積書に会社ロゴ・印鑑画像の挿入
- [ ] 棚の種別・単価マスタを管理画面から編集
